...
class DiffSchemes:
    ...
    def sadrp(self, t_plot, ylim = None, m = None):
        ...
        def F_gene(matrx_f_gene):
            ...
            F_matrx = self._get_1d_flux_basic(matrx_f_gene)
            # 0 to l-1
            S1 = np.einsum('ij,j->i', co_matrx_S1, F_matrx[:-1])
            S2 = np.einsum('ij,j->i',co_matrx_S2, F_matrx[:-1])
            S3 = np.einsum('ij,j->i',co_matrx_S3, F_matrx[:-1])
            S4 = np.einsum('ij,j->i',co_matrx_S4, F_matrx[:-1])
            C1 = np.einsum('ij,j->i',co_matrx_C1, F_matrx[:-1])
            C2 = np.einsum('ij,j->i',co_matrx_C2, F_matrx[:-1])
